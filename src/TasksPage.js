...
const TasksPage = () => {
  ...
  const handleRefresh = async () => {
    ...
    try {
      const [scoreResponse, tasksResponse, taskStatesResponse] = await Promise.all([
        axios.get(`https://task.pooldegens.meme/api/get_user_score?username=${username}`),
        axios.get('https://task.pooldegens.meme/api/get_tasks'),
        axios.get(`https://task.pooldegens.meme/api/get_task_states?username=${username}`)
      ]);
  ...
  const fetchTaskStates = async () => {
    try {
      const response = await axios.get(`https://task.pooldegens.meme/api/get_task_states?username=${username}`);
      const states = {};
  ...
  const handleSubmitEvidence = async () => {
    ...
    try {
      ...
      const response = await axios.post('https://task.pooldegens.meme/api/complete_task', {
        username,
  ...
  const handleClaimReward = async (taskId, taskScore) => {
    try {
      await axios.post('https://task.pooldegens.meme/api/claim_reward', {
        username,
        task_id: taskId,
        score: taskScore
      });
